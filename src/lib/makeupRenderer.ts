...
export function renderMakeup(
  ...
  // ── 1. Skin smoothing (subtle Gaussian blur on face region) ──────
  drawBlurredLayer(ctx, w, h, params.smoothing, "source-over", (offCtx) => {
    // Create a face-shaped mask and sample from main canvas
    offCtx.save();
    ...
    // Draw the original frame into the blurred context (it will be blurred by the filter)
